...
Le formulaire de contact envoie `POST {VITE_API_URL}/send-mail` et la
newsletter `POST {VITE_API_URL}/newsletter`.
...
├── App.tsx             Routeur + écran de chargement + ToastContainer
├── i18n.ts             Configuration i18next (langue par défaut : fr)
└── components/
...
Chaque composant conserve son propre fichier CSS (design existant préservé).
